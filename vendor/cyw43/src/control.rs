...
impl<'a> Control<'a> {
    ...
    /// Read the level of a GPIO pin on the WiFi chip.
    ///
    /// Same query the reference C driver uses (`ccgpioin`). On a Raspberry
    /// Pi Pico W, pin 2 is the USB power sense (VBUS present).
    /// (Sprig OS addition; not in upstream cyw43 0.7.0.)
    pub async fn gpio_get(&mut self, gpio_n: u8) -> bool {
        assert!(gpio_n < 3);
        let levels = self.get_iovar_u32("ccgpioin").await;
        (levels >> gpio_n) & 1 != 0
    }
