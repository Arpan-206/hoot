# cyw43 0.7.0, vendored for Sprig OS

Unmodified copy of the `cyw43` crate from crates.io, plus one method:
`Control::gpio_get`, which reads the radio chip's own GPIO pins with the
`ccgpioin` query, as the reference C driver does. On a Pico W, GPIO 2 of
the radio is the USB power sense, which the RP2040 cannot see because that
board routes VBUS there instead of to GP24.

The workspace `Cargo.toml` points the `cyw43` dependency here with
`[patch.crates-io]`. Drop this directory and the patch once upstream has a
`gpio_get`.
